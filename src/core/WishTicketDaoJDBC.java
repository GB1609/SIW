...
public class WishTicketDaoJDBC implements WishTicketDao
{
	...
	public Set<Clients> searchInterested(Ticket b)
	{
		...
		try
		{
			String query="SELECT client.username, client.password, client.lastname, " + "client.firstname, client.birthdate, client.address, client.credit " + "FROM client,desiredtickets,wishlist "
					+ "WHERE desiredtickets.ticketcode=? AND " + "wishlist.owner=client.username AND " + "desiredtickets.listcode=wishlist.listcode ";
			PreparedStatement statement=connection.prepareStatement(query);
			statement.setInt(1,cod);
			ResultSet result=statement.executeQuery();
			while (result.next())
			{
				Clients c=new Clients(result.getString("username"),result.getString("password"),result.getString("lastname"),result.getString("firstname"),result.getDate("birthdate").toLocalDate(),
						result.getString("address"),(float) result.getDouble("credit"));
				myResult.add(c);
			}
		}
		catch (Exception e)
	...
	public Set<Ticket> searchOwners(Clients c)
	{
		...
		try
		{
			String query="SELECT ticket.price, ticket.type, ticket.event, ticket.ticketcode " + "FROM ticket,desiredtickets,wishlist " + "WHERE ticket.ticketcode=desiredtickets.ticketcode AND "
