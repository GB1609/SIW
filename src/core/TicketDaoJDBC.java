...
public class TicketDaoJDBC implements TicketDao
{
	...
	public Set<Ticket> searchByEvents(int e)
	{
		...
		try
		{
			...
			while (result.next())
			{
				Ticket t=new Ticket(result.getInt("ticketcode"),result.getInt("event"),result.getDouble("price"),result.getString("type"),result.getBoolean("sell"));
				myResult.add(t);
			}
		}
		catch (Exception e1)
